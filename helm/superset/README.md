...
| envFromSecrets | list | `[]` | This can be a list of templated strings. They are loaded after `envFromSecret` in every container, init containers included, so credentials kept in an externally managed secret take precedence over the chart managed ones |
...
| extraEnv | object | `{}` | Extra environment variables that will be passed into pods, init containers included |
| extraEnvRaw | list | `[]` | Extra environment variables in RAW format that will be passed into pods, init containers included |
